...
import { conditions, disclaimer, getCondition, site } from "../data/content";
import {
  Accordion,
  Button,
  Icon,
...
export default function ConditionDetail() {
  ...
  const c = slug ? getCondition(slug) : undefined;

  usePageMeta(
    c ? `${c.title} in Darbhanga | ZivRA HEALTH` : "Condition | ZivRA HEALTH",
    c
      ? `${c.title} — understanding, symptoms and when to seek advice. Consult Dr. Adeel, urologist in Darbhanga, Bihar. Book: ${site.phone}.`
      : undefined
  );

  if (!c) return <Navigate to="/conditions" replace />;
